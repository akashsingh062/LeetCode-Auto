...
function handlePushError(err) {
  ...
  if (err.message === 'leetcode_auto token is undefined') {
    ...
    message =
      'LeetCode-Auto: GitHub authentication failed (401 Unauthorized). Please check your token or re-authenticate.';
  } else if (err.message === '403') {
    message =
      'LeetCode-Auto: GitHub access forbidden (403 Forbidden). Please make sure your token has the "repo" scope checked.';
  } else if (err.message === '404') {
    message =
      'LeetCode-Auto: GitHub repository not found (404 Not Found). Please verify that the linked repository exists.';
  } else if (err.message) {
    message = `LeetCode-Auto: ${err.message}`;
  }
  alert(message);
}
...
LeetCodeV2.prototype.injectAndListen = function () {
  window.addEventListener('message', event => {
    ...
    if (data.type === 'leetcodeAutoSubmissionId') {
      console.log('[LeetCode-Auto] Received submission ID:', data.submissionId);
      this.processSubmission(data.submissionId);
    } else if (data.type === 'leetcodeAutoSolutionPost') {
      const { questionSlug, content, title } = data;
      console.log('LeetCode-Auto: Received solution post event:', data);
